...
func TestS9_RefreshSession_RecoversAcrossStopStart(t *testing.T) {
	...
	for _, d := range allDrivers {
		t.Run(d.name, func(t *testing.T) {
			...
			// the cluster's live session against the cluster's current host:port,
			// then wraps it with the same adapter under test.
			...
			// port — the existing session is now permanently broken.
			require.NoError(t, a.Stop(ctx, 30*time.Second))
			require.NoError(t, a.Start(ctx))

			// Phase 3: invoke the feature under test.
			...
			// Phase 5: the client is still fully usable against B as well.
			require.NoError(t,
