...
// validatePayload rejects a payload whose target cluster is unknown or
// whose arguments the replay wire format cannot carry, so both replayers
// give the same answer at enqueue regardless of which one is configured.
// Encodability has exactly one definition, the encoder itself: the memory
// backend runs it as a dry run.
func validatePayload(payload types.ReplayPayload) error {
	if err := validateTargetCluster(payload.TargetCluster); err != nil {
		return err
	}
	if _, err := encodeArgs(payload.Args); err != nil {
		return err
	}
	for _, stmt := range payload.BatchStatements {
		if _, err := encodeArgs(stmt.Args); err != nil {
			return err
		}
	}
