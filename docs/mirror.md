...
| Args | `[]any` args (and batch entries' args) are copied element-by-element synchronously **before** `Exec` returns to the caller, but only `[]byte` values are deep-copied into a fresh buffer — reusing or pooling a `[]byte` arg after `Exec` returns is safe. Every other reference type (`[]string`, `map[K]V`, and other nested Cassandra list/set/map column values) is copied by reference: the mirror payload shares the caller's slice or map header. Do not mutate or reuse one of those values after `Exec` returns until the mirror leg has run; pass an independent copy per call if you need to reuse the underlying buffer. |
