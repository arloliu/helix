...
| `helix_read_errors_total{cluster}` climbing on one cluster while `helix_failover_total` and the circuit breaker's state both stay flat | That cluster's coordinator is rejecting the statement itself (`types.ErrStatementRejected`): a schema migration has not reached it yet, or the query is malformed for it | Check the schema on that cluster, or the statement, not the connection — the cluster is healthy and reachable, so restarting it does nothing. A rejection is not a cluster-health signal, so Helix deliberately does not fail over on it, even though during a staggered schema migration the other cluster may already have the table |
